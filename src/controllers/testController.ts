...
export default class testController {
  ...
  static async putExample(req: FastifyRequest, reply: FastifyReply) {
    try {
      // Your logic for handling the PUT request
      const requestData = req.body;
      const response = { message: 'Received PUT request', data: requestData };
      reply.send(response);
    } catch (error) {
      // Handle errors
      console.error(error);
